...
>(
  ...
  listener: CellIdsListener<ExtractSchemasFromStore<Store>, TableIdOrNull, RowIdOrNull>,
  ...
  return useListener(
    ...
    store => store.addCellIdsListener(toValue(tableId), toValue(rowId), listener as any, toValue(mutator)),
    options,
  )
}
