...
>(
  ...
  listener: RowListener<ExtractSchemasFromStore<Store>, TableIdOrNull, RowIdOrNull>,
  ...
  return useListener(
    ...
    store => store.addRowListener(toValue(tableId), toValue(rowId), listener as any, toValue(mutator)),
    options,
  )
}
